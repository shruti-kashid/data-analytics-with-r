...
tail(df)#last 6 rows
tail(df, n=3) #last 3 rows
View(df) #to view data set in the form of table in diffrent tab
class(df) # DF

(m1<-matrix(1:12, nrow =3))#matrix and its class
class(m1)

(v1<-c(1:8))#vctor and its class
v1
class(v1)
